    ...
    <div className="modal-bg" role="dialog" aria-modal="true" aria-label="Sign in" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: "#282828", borderRadius: 14, padding: 32, width: 400, maxWidth: "94vw", textAlign: "center", position: "relative" }}>
        ...
        <div style={{ textAlign: "left", marginBottom: 16 }}>
          ...
          <input className="finput" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" onKeyDown={(e) => { if (e.key === "Enter") onLogin(); }} />
        </div>

        <button onClick={onLogin} style={{ width: "100%", background: "#3ea6ff", border: "none", color: "#000", padding: 11, borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer", marginBottom: 12 }}>
          Sign in
        </button>
        ...
        <button onClick={onLogin} style={{ width: "100%", background: "var(--bg3)", border: "0.5px solid var(--bg4)", color: "var(--text)", padding: 11, borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
          <span style={{ fontSize: 16, fontWeight: 700, fontFamily: "serif", color: "#4285f4" }}>G</span>
          Continue with Google
        </button>

        <p style={{ fontSize: 11, color: "var(--dim)", marginTop: 14 }}>
          By continuing you agree to ViewTube's Terms &amp; Privacy Policy
        </p>
      </div>
    </div>
  );
